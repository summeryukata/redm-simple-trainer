...
{
    internal class SpawnPed : Drawing
    {
        public static async Task SpawnModel(string name)
        {
            int model = GenHash(name);
            int player = Function.Call<int>(Hash.PLAYER_PED_ID);
            Vector3 playerPos = Function.Call<Vector3>(Hash.GET_ENTITY_COORDS, player);

            Debug.WriteLine(playerPos.ToString());

            await Main.PerformRequest(model);

            Debug.WriteLine($"Spawning a {name} ({model})");

            int createdPed = Function.Call<int>(Hash.CREATE_PED, model, playerPos.X, playerPos.Y, playerPos.Z + 2, 0.0, true, false);
            Function.Call((Hash)0x283978A15512B2FE, createdPed, true);
            Function.Call(Hash.TASK_WANDER_STANDARD, createdPed, 10f, 10);
        }

        public static async Task Draw()
        {
            SetMenuTitle("Spawn Ped", "KEKW");
